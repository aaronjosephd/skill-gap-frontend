...
<div class="navbar bg-base-100 shadow-md px-4 md:px-8">
  <div class="navbar-start">
    <div class="lg:hidden">
      <button on:click={toggleMenu} class="btn btn-ghost">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7" /></svg>
      </button>
      {#if isMenuOpen}
        <ul class="menu menu-sm absolute mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52">
          <li><a href="/market" on:click={() => isMenuOpen = false}>Market Insights</a></li>
          <li><a href="/analyze" on:click={() => isMenuOpen = false}>Resume Analysis</a></li>
          <li><button on:click={() => { isMenuOpen = false; isAboutModalOpen = true; }}>About</button></li>
        </ul>
      {/if}
    </div>
